...
<head>
    ...
    <style>
        ...
        /* CSS Pagination Dark */
        .dark-pagination nav div[class*="flex"] { justify-content: flex-end; }
...
<body class="min-h-screen bg-gradient-to-b from-page-bg-start via-[#be185d] to-slate-900 text-white pb-24 p-6 md:p-10 font-sans">
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
                ...
                <div class="flex gap-2 text-xs font-semibold text-white">
                    ...
                    <span class="bg-emerald-500 px-3 py-1 rounded-full shadow-sm border border-white/20">Aktif</span>
            ...
            <form action="{{ route('admin.users.index') }}" method="GET" class="w-full md:w-auto flex flex-col md:flex-row gap-2">
                ...
                <div class="relative">
                    <select name="filter" class="appearance-none w-full md:w-auto bg-beige-main text-gray-700 text-sm font-medium border border-beige-dark py-2.5 pl-4 pr-10 rounded-lg focus:outline-none focus:ring-2 focus:ring-btn-maroon cursor-pointer shadow-sm">
                        <option value="name" {{ request('filter') == 'name' ? 'selected' : '' }}>Cari Nama</option>
                        <option value="nik" {{ request('filter') == 'nik' ? 'selected' : '' }}>Cari NIK</option>
                        <option value="email" {{ request('filter') == 'email' ? 'selected' : '' }}>Cari Email</option>
                    </select>
                ...
                <div class="relative w-full md:w-72">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Ketik kata kunci & Enter..."
        ...
        @endif
        @if(session('error'))
            <div id="alertBoxErr" class="mb-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded shadow-sm">
                {{ session('error') }}
            </div>
        @endif
                    ...
                    <tbody class="divide-y divide-gray-300/50">
                        ...
                        <tr class="hover:bg-white/40 transition-colors group">
                            ...
                            <td class="px-6 py-4 align-middle text-center">
                                @if($user->is_active)
                                    <span class="inline-flex items-center gap-1 bg-green-100 text-green-700 text-[11px] font-bold px-2.5 py-1 rounded-full border border-green-200"><span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> Aktif</span>
                                @else
                                    <span class="inline-flex items-center gap-1 bg-gray-100 text-gray-600 text-[11px] font-bold px-2.5 py-1 rounded-full border border-gray-300"><span class="w-1.5 h-1.5 rounded-full bg-gray-400"></span> Nonaktif</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 align-middle text-center">
                                <form method="POST" action="{{ $user->is_active ? route('admin.users.deactivate', $user->id) : route('admin.users.activate', $user->id) }}" onsubmit="return confirmAction(event, '{{ $user->is_active ? 'nonaktifkan' : 'aktifkan' }}', '{{ $user->name }}')">
                                    @csrf @method('PUT')
                                    @if($user->is_active)
                                        <button type="submit" class="w-32 bg-transparent border border-status-pink text-status-pink hover:bg-status-pink hover:text-white transition-all font-semibold rounded-lg text-xs px-3 py-2 shadow-sm"><i class="fas fa-ban mr-1"></i> Nonaktifkan</button>
                                    @else
                                        <button type="submit" class="w-32 bg-transparent border border-status-green text-status-green hover:bg-status-green hover:text-white transition-all font-semibold rounded-lg text-xs px-3 py-2 shadow-sm"><i class="fas fa-check mr-1"></i> Aktifkan</button>
                                    @endif
                                </form>
                            </td>
                        </tr>
