    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        'page-bg-start': '#ec4899', // WAJIB ADA
                        'page-bg-end': '#1e3a8a',
                        'card-beige': '#ebdccb',
                        'btn-pink': '#fb7185',
                        'btn-hover': '#f43f5e',
    ...
    <main class="px-6 md:px-12 max-w-5xl mx-auto -mt-4">
        ...
        <div class="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
            <div class="w-full md:w-auto">
                <h2 class="text-xl font-semibold text-white/90 mb-1">Daftar Laporan Saya</h2>
        ...
        <div class="space-y-6" id="reportListContainer">
                ...
                <div class="report-item bg-card-beige rounded-2xl p-6 shadow-xl text-gray-800 relative group transition hover:shadow-2xl">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start mb-4 gap-2">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            @if($report->status == 'pending')
                                <div class="flex items-center gap-1 bg-white/50 p-1 rounded-lg border border-white/60">
                                    <button onclick="openEditModal(this)"
                                        data-id="{{ $report->id }}"
                                        data-title="{{ $report->title }}"
                                        data-desc="{{ $report->description }}"
                                        data-loc="{{ $report->location }}"
                                        data-cat="{{ $report->category }}"
                                        data-photo="{{ $report->photo ? asset('storage/' . $report->photo) : '' }}"
                                        data-url="{{ route('reports.update', $report->id) }}"
                                        class="w-7 h-7 flex items-center justify-center bg-orange-100 text-orange-600 rounded hover:bg-orange-200 transition" title="Edit">
                                        <i class="fas fa-edit text-xs"></i>
                                    </button>

                                    <form method="POST" action="{{ route('reports.destroy', $report->id) }}" onsubmit="return confirm('Hapus laporan ini?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="w-7 h-7 flex items-center justify-center bg-red-100 text-red-600 rounded hover:bg-red-200 transition" title="Hapus">
                        ...
                        <div class="mb-4">
                            ...
                            <a href="{{ asset('storage/' . $report->photo) }}" target="_blank" class="block w-full md:w-64 h-40 rounded-lg overflow-hidden border border-gray-300 relative group shadow-sm">
                                ...
                                <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <span class="text-white text-xs font-bold flex items-center"><i class="fas fa-search-plus mr-1"></i> Lihat Penuh</span>
                                </div>
                            </a>
                        </div>
                    @endif
                    ...
                    @if($report->feedback)
                        <div class="mt-2 bg-white/60 p-3 rounded-lg border-l-4 border-blue-500 text-sm text-gray-700">
                            <span class="font-bold text-blue-600 block mb-1">Tanggapan Petugas:</span> "{{ $report->feedback }}"
                            @if ($report->admin_photo)
                                <a href="{{ asset('storage/' . $report->admin_photo) }}" target="_blank" class="text-xs text-green-600 font-bold hover:underline flex items-center mt-2"><i class="fas fa-image mr-1"></i> Lihat Foto Bukti Petugas</a>
                            @endif
                        </div>
                    @endif
                </div>
            @empty
                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-10 text-center border border-white/20" id="emptyState">
                    <p class="text-white/70">Belum ada laporan. Klik tombol "Buat Laporan Baru" di atas.</p>
                </div>
            @endforelse
            ...
            <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 border-b border-gray-100 shrink-0 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                        <i class="fas fa-plus-circle text-green-500"></i> Buat Laporan Baru
                    </h3>
                    <button onclick="closeModal('createReportModal')" class="text-gray-400 hover:text-gray-600">
                ...
                <div class="p-6 overflow-y-auto custom-scroll">
                    <form method="POST" action="{{ route('reports.store') }}" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Judul Laporan</label>
                            <input type="text" name="title" required class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-lg p-2.5">
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Kategori</label>
                                <select name="category" class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-lg p-2.5">
                            ...
                            <div>
                                ...
                                <input type="text" name="location" required class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-lg p-2.5">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Deskripsi</label>
                            <textarea name="description" rows="4" required class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-lg p-2.5"></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Foto Bukti (Opsional)</label>
                            <input type="file" name="photo" class="w-full text-xs text-gray-500 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none">
                        </div>

                        <div class="pt-2">
                            <button type="submit" class="w-full bg-btn-pink hover:bg-btn-hover text-white font-bold py-2.5 rounded-lg shadow-md transition-all">
                                Kirim Laporan
                            </button>
                        </div>
                    </form>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 border-b border-gray-100 shrink-0 flex justify-between items-center">
                    ...
                    <button onclick="closeModal('editReportModal')" class="text-gray-400 hover:text-gray-600">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                </div>
                ...
                <div class="p-6 overflow-y-auto custom-scroll">
                    <form id="editReportForm" method="POST" action="" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="editTitle" required class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2 text-sm text-gray-800">
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Kategori</label>
                                <select name="category" id="editCategory" class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2 text-sm text-gray-800">
                                    <option value="infrastruktur">Infrastruktur</option>
                                    <option value="kebersihan">Kebersihan</option>
                                    <option value="keamanan">Keamanan</option>
                                    <option value="lainnya">Lainnya</option>
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-600 mb-1">Lokasi</label>
                                <input type="text" name="location" id="editLocation" required class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2 text-sm text-gray-800">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">Deskripsi</label>
                            <textarea name="description" id="editDescription" rows="3" required class="w-full bg-gray-50 border border-gray-300 rounded-lg p-2 text-sm text-gray-800"></textarea>
                        </div>

                        <div id="editPhotoContainer" class="hidden mb-2">
                            <label class="block text-xs font-bold text-gray-600 mb-1">Foto Saat Ini</label>
                            <div class="w-full h-32 rounded-lg overflow-hidden border border-gray-300 bg-gray-100">
                                <img id="editPhotoPreview" src="" class="w-full h-full object-cover">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">Ganti Foto (Opsional)</label>
                            <input type="file" name="photo" class="w-full text-xs text-gray-500 border border-gray-300 rounded-lg p-1 bg-gray-50">
                        </div>

                        <div class="flex gap-2 pt-2">
                            <button type="button" onclick="closeModal('editReportModal')" class="w-1/2 py-2 border border-gray-300 rounded-lg text-gray-600 text-sm font-bold hover:bg-gray-100">Batal</button>
                            <button type="submit" class="w-1/2 py-2 bg-orange-500 text-white rounded-lg text-sm font-bold hover:bg-orange-600 shadow-md">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
